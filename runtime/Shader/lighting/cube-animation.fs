...
void main() {
	// 环境光
	vec3 ambient = light.ambient * texture(material.diffuse, TexCoords + vec2(texSOffset, texTOffset)).rgb;

	// 漫反射
	vec3 lightDir = normalize(vec3(light.position - FragPos));
	vec3 norm = normalize(Normal);
	float diff = max(dot(norm, lightDir), 0.0);
	vec3 diffuse = light.diffuse * diff * texture(material.diffuse, TexCoords + vec2(texSOffset, texTOffset)).rgb;

	// 镜面光
	vec3 viewDir = normalize(viewPos - FragPos);
	vec3 reflectDir = normalize(reflect(-lightDir, norm));
	float spec = pow(max(dot(reflectDir, viewDir), 0.0), material.shineness);
	vec3 specular = light.specular * abs(spec) * material.specular;
